//
//  PrivateFeedViewController.swift
//  Finder
//
...
class PrivateFeedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UIActionSheetDelegate, UIPopoverPresentationControllerDelegate {
    ...
    @IBAction func privatenextpostbutton(sender: AnyObject) {
        
        let button = sender as! UIButton
        let view = button.superview!
        let cell = view.superview as! PrivateFeedControllerCell
        ...
        if row!+1 == tableView.numberOfRowsInSection(indexPath!.section){
            ...
            //self.performSegueWithIdentifier("privaterefreshsegue", sender: self)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "showcommentsegue"){
            ...
            query.whereKey("privatename", equalTo: lastviewed)
            ...
            let navVC = segue.destinationViewController as! DetailViewController
            //let detailVC = navVC.topViewController as! DetailViewController
            navVC.yak = obj
        }
        
        
        if(segue.identifier == "privatepostimage1segue"){
            let indexPath = self.tableView.indexPathForSelectedRow()
            ...
            let navVC = segue.destinationViewController as! PrivateSinglePostViewController
            //let detailVC = navVC.topViewController as! PackFeedViewController
            navVC.lastviewed = lastviewed1
        ...
        if(segue.identifier == "privatepostimage2segue"){
            let indexPath = self.tableView.indexPathForSelectedRow()
            //let indes = tableView.indexPathForSelectedRow()
            //var lastviewed = userids[indexPath!.row]
            //let destination = segue.destinationViewController as! UINavigationController
            ...
            let lastviewed1 = lastviewed
            //let lastviewed = userids[indexPath!.row]
            let navVC = segue.destinationViewController as! PrivateDoublePostViewController
            //let detailVC = navVC.topViewController as! PackFeedViewController
            navVC.lastviewed = lastviewed1
        ...
        if(segue.identifier == "privaterefreshsegue"){
            let indexPath = self.tableView.indexPathForSelectedRow()
            //let indes = tableView.indexPathForSelectedRow()
            //var lastviewed = userids[indexPath!.row]
            //let destination = segue.destinationViewController as! UINavigationController
    ...
    func actionSheet(actionSheet: UIActionSheet, clickedButtonAtIndex buttonIndex: Int) {
        ...
        if buttonIndex == 1 { // Post One Pic
            ...
            self.performSegueWithIdentifier("privatepostimage1segue", sender: self)
            //privatepostimage2segue
            
            
        } else if buttonIndex == 2 { // Post 2 Pic
            
            self.performSegueWithIdentifier("privatepostimage2segue", sender: self)
    ...
    func addTapped (sender:UIButton) {
        self.performSegueWithIdentifier("showcommentsegue", sender: self)
    }
    ...
    override func viewDidLoad() {
        ...
        let backButton = UIBarButtonItem(title: "\(lastviewed)", style: UIBarButtonItemStyle.Plain, target: self, action: nil)
        backButton.setTitleTextAttributes([NSFontAttributeName: UIFont(name: "HelveticaNeue-Light", size: 16)!], forState: UIControlState.Normal)
        navigationItem.backBarButtonItem = backButton
        
        
        
        var rightAddBarButtonItem:UIBarButtonItem = UIBarButtonItem(image: UIImage(named: "chat.png"), style: UIBarButtonItemStyle.Plain, target: self, action: "addTapped:")
        
        var rightSearchBarButtonItem:UIBarButtonItem = UIBarButtonItem(image: UIImage(named: "write-simple-7.png"), style: UIBarButtonItemStyle.Plain, target: self, action: "searchTapped:")
        
        self.navigationItem.setRightBarButtonItems([rightSearchBarButtonItem,rightAddBarButtonItem], animated: true)
        
        
        //testy.frame.size.height = 25
        ...
        //tableView.allowsSelection = false
            ...
            query.whereKey("privatename", equalTo: self.lastviewed)
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let myCell2 = tableView.dequeueReusableCellWithIdentifier("privatecell", forIndexPath: indexPath) as! PrivateFeedControllerCell
