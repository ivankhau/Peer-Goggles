...
//  Finder
//
//  Created by Ivan Khau on 9/10/15.
...
class resultsviewcontroller: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    var vote1label:UILabel = UILabel()
    ...
    @IBAction func resultstoprofile(sender: AnyObject) {
        
        let button = sender as! UIButton
        let view = button.superview!
        let cell = view.superview as! resultsviewcontrollercell
        ...
        query.getObjectInBackgroundWithId(messages[row!]) { (toUserObject, NSError) -> Void in
            if toUserObject == nil {
                self.displayAlert("Profile is Hidden", message: "Profiles are hidden until the user creates a non-anonymous post.")
            } else {
            self.userObject = toUserObject
            self.performSegueWithIdentifier("resulttoprofile", sender: self)
            }
        }
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if(segue.identifier == "resulttoprofile"){
            ...
            destinationViewController.toUser = userObject
    ...
    func refresh(){
        ...
        self.singlevoteresult1.removeAll(keepCapacity: true)
        self.singlevoteresult2.removeAll(keepCapacity: true)
        self.usernames3.removeAll(keepCapacity: true)
        self.messages.removeAll(keepCapacity: true)
        self.usernames.removeAll(keepCapacity: true)
        self.usernames2.removeAll(keepCapacity: true)
        self.imageFiles.removeAll(keepCapacity: true)
        self.barwidth1.removeAll(keepCapacity: false)
        self.barwidth2.removeAll(keepCapacity: false)
        self.alias.removeAll(keepCapacity: false)
        self.questiontype.removeAll(keepCapacity: false)
        ...
        //if whichsort == "All" {
            ...
            query.whereKey("ViewedUsers", equalTo: PFUser.currentUser()!.objectId!)
            ...
            query.findObjectsInBackgroundWithBlock({ (objects, error) -> Void in
                ...
                if error == nil {
                    ...
                    self.county = objects.count
                    if objects.count == 0 {
                        self.activityIndicator.stopAnimating()
                        self.topbar.userInteractionEnabled = true
                        self.refresher.endRefreshing()
                        self.tableView.reloadData()
                        self.resultslabel.text = "See Results After Voting\n(Pull to Refresh)"
                        self.resultslabel.font = UIFont(name: "AvenirNext-Medium", size: 12)
                    ...
                    if let objects = objects  {
                        ...
                        self.resultslabel.font = UIFont(name: "AvenirNext-Medium", size: 18)
            ...
            getFollowedUsersQuery.findObjectsInBackgroundWithBlock { (objects, error) -> Void in
                ...
                if let objects = objects {
                    ...
                    for object in objects {
                        ...
                        query2.whereKey("ViewedUsers", equalTo: PFUser.currentUser()!.objectId!)
                        query2.whereKey("userId", equalTo: followedUser)
                        query2.whereKey("alias", notEqualTo: "Anonymous")
    ...
    /*
    @IBAction func fetchlast(sender: AnyObject) {
        self.singlevoteresult1.removeAll(keepCapacity: true)
        self.singlevoteresult2.removeAll(keepCapacity: true)
        self.usernames3.removeAll(keepCapacity: true)
        self.messages.removeAll(keepCapacity: true)
    ...
    override func viewDidLoad() {
        ...
        query1.whereKey("following", equalTo: toUser.objectForKey("userId"))
        ...
        query2.whereKey("objectId", equalTo: toUser.objectForKey("userId"))
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let myCell2 = tableView.dequeueReusableCellWithIdentifier("resultscell", forIndexPath: indexPath) as! resultsviewcontrollercell
        ...
        //myCell2.barfront1.addConstraint(NSLayoutConstraint(item: myCell2.barfront1, attribute: NSLayoutAttribute.Width, relatedBy: NSLayoutRelation.Equal, toItem: nil, attribute: NSLayoutAttribute.NotAnAttribute, multiplier: 1, constant: myCell2.barback1.frame.width * barwidth1[indexPath.row] * 0.01))
        
        //myCell2.barfront2.addConstraint(NSLayoutConstraint(item: myCell2.barfront2, attribute: NSLayoutAttribute.Width, relatedBy: NSLayoutRelation.Equal, toItem: nil, attribute: NSLayoutAttribute.NotAnAttribute, multiplier: 1, constant: myCell2.barback2.frame.width * barwidth2[indexPath.row] * 0.01))
        ...
        //myCell2.barfront1.frame.size.width = myCell2.barback1.frame.size.width * barwidth1[indexPath.row] * 0.01
        //myCell2.barfront2.frame.size.width = myCell2.barback2.frame.size.width * barwidth2[indexPath.row] * 0.01
        
        //vote1label.frame = CGRectMake(137, 67, myCell2.barback2.frame.height, myCell2.barback2.frame.width * barwidth2[indexPath.row] * 0.01)
        ...
        myCell2.profilebutton.setTitle(alias[indexPath.row], forState: .Normal)
        
        if alias[indexPath.row] == "Anonymous" {
            myCell2.profilebutton.hidden = true
        } else {
            myCell2.profilebutton.hidden = false
        }
        ...
        rectShape.bounds = myCell2.barfront1.frame
        rectShape.position = myCell2.barfront1.center
        rectShape.path = UIBezierPath(roundedRect: myCell2.barfront1.bounds, byRoundingCorners: .BottomLeft | .TopLeft, cornerRadii: CGSize(width: 10, height: 10)).CGPath
        
        //self.barfront.layer.backgroundColor = UIColor.greenColor().CGColor
        //Here I'm masking the textView's layer with rectShape layer
        myCell2.barfront1.layer.mask = rectShape
